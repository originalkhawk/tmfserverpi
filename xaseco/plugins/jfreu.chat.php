...
function chat_jfreu($aseco, $command)
{
	...
	if ($command['params'][0] == 'help')
	{
		...
		showHelp($admin, $jfreu->admin_commands, 'Jfreu admin', true, false);
	}
	...
	elseif ($command['params'][0] == 'helpall')
	{
		...
		showHelp($admin, $jfreu->admin_commands, 'Jfreu admin', true, true, 0.45);
	}
	...
	elseif ($command['params'][0] == 'autochangename')
	{
		...
		else
		{
			$message = $yel.'> '.$blu.'AutoChangeName is: '.$whi.
			           ($jfreu->autochangename ? 'ON' : 'OFF').$blu.'.';
	...
	elseif ($command['params'][0] == 'setrank')
	{
		if ($command['params'][1] != '')
		{
			if (strtoupper($command['params'][1]) == 'ON')
			{
				...
				{
					$servername = $jfreu->servername . $jfreu->top . $jfreu->limit;
					$aseco->client->query('SetServerName', $servername);
				}
			}
			elseif (strtoupper($command['params'][1]) == 'OFF')
			{
				$jfreu->ranklimit = false;
				// log console message
				...
				if ($jfreu->autochangename)
				{
					$servername = $jfreu->servername . ' NoLimit';
					$aseco->client->query('SetServerName', $servername);
				}
			}
		}
	...
	elseif ($command['params'][0] == 'setlimit')
	{
		...
		{
			$message = $yel.'> '.$blu.'RankLimit value is: '.$whi.$jfreu->limit.$blu.'.';
			$aseco->client->query('ChatSendServerMessageToLogin', $aseco->formatColors($message), $login);
		}
	}
		...
		if ($command['params'][1] != '')
		{
			...
			elseif (strtoupper($command['params'][1]) == 'OFF')
			{
				...
				$message = formatText('{#server}>> {#message}{1}$z$s {#highlite}{2}$z$s{#message} set AutoRank: {#highlite}{3}{#message}.',
				                      $chattitle, $admin->nickname, 'OFF');
				$aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));
				set_ranklimit($aseco, $jfreu->limit, 0);
				write_config_xml($aseco);
			}
		}
		else
	...
	elseif ($command['params'][0] == 'offset')
	{
		if ($command['params'][1] != '')
		{
			...
			{
				$jfreu->offset = $command['params'][1];
				// log console message
				$aseco->console('{1} [{2}] set autorank offset: {3}', $logtitle, $login, $jfreu->offset);
				// show chat message
				$message = formatText('{#server}>> {#message}{1}$z$s {#highlite}{2}$z$s{#message} set AutoRank Offset to: {#highlite}{3}{#message}.',
				                      $chattitle, $admin->nickname, $jfreu->offset);
				$aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));
				autorank($aseco, $command);
				write_config_xml($aseco);
			}
		...
		{
			$message = $yel.'> '.$blu.'AutoRank Offset value is: '.$whi.$jfreu->offset.$blu.'.';
			$aseco->client->query('ChatSendServerMessageToLogin', $message, $login);
		}
	}
	...
	elseif ($command['params'][0] == 'hardlimit')
	{
		if ($command['params'][1] != '')
		{
			if (is_numeric($command['params'][1]) &&
			    $command['params'][1] >= 0 && $command['params'][1] < 2000000)
			{
				$jfreu->hardlimit = $command['params'][1];
				// log console message
				$aseco->console('{1} [{2}] set hardlimit: {3}', $logtitle, $login, $jfreu->hardlimit);
				// show chat message
				$message = formatText('{#server}>> {#message}{1}$z$s {#highlite}{2}$z$s{#message} set HardLimit to: {#highlite}{3}{#message}.',
				                      $chattitle, $admin->nickname, $jfreu->hardlimit);
				$aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));
				write_config_xml($aseco);
			}
			else
		...
		elseif ($jfreu->hardlimit > 0)
		{
			$message = $yel.'> '.$blu.'HardLimit value is: '.$whi.$jfreu->hardlimit.$blu.'.';
			$aseco->client->query('ChatSendServerMessageToLogin', $message, $login);
		}
		else
		{
	...
	elseif ($command['params'][0] == 'autorankminplayers')
	{
		if ($command['params'][1] != '')
		{
			...
			{
				$jfreu->autorankminplayers = $command['params'][1];
				// log console message
				$aseco->console('{1} [{2}] set autorankminplayers: {3}', $logtitle, $login, $jfreu->autorankminplayers);
				// show chat message
				$message = formatText('{#server}>> {#message}{1}$z$s {#highlite}{2}$z$s{#message} set AutoRankMinPlayer to: {#highlite}{3}{#message}.',
				                      $chattitle, $admin->nickname, $jfreu->autorankminplayers);
				$aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));
				autorank($aseco, $command);
				write_config_xml($aseco);
			}
		...
		{
			$message = $yel.'> '.$blu.'AutoRankMinPlayer value is: '.$whi.$jfreu->autorankminplayers.$blu.'.';
			$aseco->client->query('ChatSendServerMessageToLogin', $message, $login);
		}
	}
	...
	elseif ($command['params'][0] == 'maxplayers')
	{
		if ($command['params'][1] != '')
		{
			...
			{
				$jfreu->maxplayers = $command['params'][1];
				// log console message
				$aseco->console('{1} [{2}] set maxplayers: {3}', $logtitle, $login, $jfreu->maxplayers);
				// show chat message
				$message = formatText('{#server}>> {#message}{1}$z$s {#highlite}{2}$z$s{#message} set MaxPlayers to: {#highlite}{3}{#message}.',
				                      $chattitle, $admin->nickname, $jfreu->maxplayers);
				$aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));
				write_config_xml($aseco);
			}
			else
		...
		{
			$message = $yel.'> '.$blu.'MaxPlayers value is: '.$whi.$jfreu->maxplayers.$blu.'.';
			$aseco->client->query('ChatSendServerMessageToLogin', $message, $login);
		}
	}
	...
	elseif ($command['params'][0] == 'listlimits')
	{
		if ($aseco->server->getGame() == 'TMN') {
			...
			$limits .= '  $gRank limiting            : {#black}' . ($jfreu->ranklimit ? 'ON' : 'OFF') . LF;
			$limits .= '  $gRank limit                 : {#black}' . $jfreu->limit . LF;
			$limits .= '  $gHard limit                  : {#black}' . $jfreu->hardlimit . LF;
			$limits .= '  $gAuto ranking            : {#black}' . ($jfreu->autorank ? 'ON' : 'OFF') . LF;
			$limits .= '  $gAutorank offset       : {#black}' . $jfreu->offset . LF;
			$limits .= '  $gAutorank limit           : {#black}' . $jfreu->autolimit . LF;
			$limits .= '  $gAutorank minplayers: {#black}' . $jfreu->autorankminplayers . LF;
			$limits .= '  $gAutorank VIP            : {#black}' . ($jfreu->autorankvip ? 'ON' : 'OFF') . LF;
			$limits .= '  $gMaxplayers HiRank  : {#black}' . $jfreu->maxplayers . LF;
			$limits .= '  $gKickHiRank               : {#black}' . ($jfreu->kickhirank ? 'ON' : 'OFF');
			...
			$limits = array();
			$limits[] = array('Rank limiting', '{#black}' . ($jfreu->ranklimit ? 'ON' : 'OFF'));
			$limits[] = array('Rank limit', '{#black}' . $jfreu->limit);
			$limits[] = array('Hard limit', '{#black}' . $jfreu->hardlimit);
			$limits[] = array('Auto ranking', '{#black}' . ($jfreu->autorank ? 'ON' : 'OFF'));
			$limits[] = array('Autorank offset', '{#black}' . $jfreu->offset);
			$limits[] = array('Autorank limit', '{#black}' . $jfreu->autolimit);
			$limits[] = array('Autorank minplayers', '{#black}' . $jfreu->autorankminplayers);
			$limits[] = array('Autorank VIP', '{#black}' . ($jfreu->autorankvip ? 'ON' : 'OFF'));
			$limits[] = array('Maxplayers HiRank', '{#black}' . $jfreu->maxplayers);
			$limits[] = array('KickHiRank', '{#black}' . ($jfreu->kickhirank ? 'ON' : 'OFF'));
	...
	elseif ($command['params'][0] == 'players')
	{
		...
		// use online list?
		if ($onlineonly) {
			foreach ($jfreu->playerlist as $pl => $entry) {
			...
			foreach ($jfreu->vip_list as $pl) {
				...
				if ($pl != '' && !array_key_exists($pl, $playerlist) &&
					...
					$playerlist[$pl] = $jfreu->playerlist[$pl];
				}
			}
		}
	...
	elseif ($command['params'][0] == 'unspec' && $command['params'][1] != '')
	{
		...
		if (isset($jfreu->playerlist[$target->login]) &&
		    $jfreu->playerlist[$target->login]->speconly)
		{
			$jfreu->playerlist[$target->login]->speconly = false;
			$jfreu->playerlist[$target->login]->isvip = true;
			// log console message
	...
	elseif ($command['params'][0] == 'addvip' && $command['params'][1] != '')
	{
		...
		if (in_array($target->login, $jfreu->vip_list))
		{
			$message = $yel.'> '.$blu.'Login: '.$whi.$target->login.$blu.' is already in VIP list.';
			$aseco->client->query('ChatSendServerMessageToLogin', $message, $login);
			return;
		}
		$jfreu->vip_list[] = $target->login;
	...
	elseif ($command['params'][0] == 'removevip' && $command['params'][1] != '')
	{
		...
		if (($i = array_search($target->login, $jfreu->vip_list)) === false)
		{
		...
		if (isset($jfreu->playerlist[$target->login]))
		{
			$jfreu->playerlist[$target->login]->isvip = false;
		}
	}
	...
	{
		$team = $command['params'][1];
		if (($i = array_search($team, $jfreu->vip_team_list)) === false)
		{
			$message = $yel.'> '.$blu.'Team: '.$whi.$team.$blu.' is not in VIP_Team list.';
			$aseco->client->query('ChatSendServerMessageToLogin', $message, $login);
			return;
		}
		$jfreu->vip_team_list[$i] = '';
		write_lists_xml($aseco);
		// log console message
	...
	elseif ($command['params'][0] == 'listvips')
	{
		...
		if ($aseco->server->getGame() == 'TMN') {
			...
			$lines = 0;
			$admin->msgs[0] = 1;
			foreach ($jfreu->vip_list as $player) {
				if ($player != '') {
					$plarr = array();
					$plarr['login'] = $player;
					$admin->playerlist[] = $plarr;
	...
	elseif ($command['params'][0] == 'listvipteams')
	{
		...
		if ($aseco->server->getGame() == 'TMN') {
			...
			$lines = 0;
			$admin->msgs[0] = 1;
			foreach ($jfreu->vip_team_list as $team) {
				if ($team != '') {
					$plarr = array();
					$plarr['login'] = $team;
					$admin->playerlist[] = $plarr;
	...
	elseif ($command['params'][0] == 'badwordsnum')
	{
		if ($command['params'][1] != '' && is_numeric($command['params'][1]))
		{
			...
			{
				$jfreu->badwordsnum = $command['params'][1];
				// log console message
				$aseco->console('{1} [{2}] set badwordsnum: {3}', $logtitle, $login, $jfreu->badwordsnum);
				// show chat message
				$message = formatText('{#server}>> {#message}{1}$z$s {#highlite}{2}$z$s{#message} set BadWordsNum to: {#highlite}{3}{#message}.',
				                      $chattitle, $admin->nickname, $jfreu->badwordsnum);
				$aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));
				write_config_xml($aseco);
			}
		}
		...
		{
			$message = $yel.'> '.$blu.'BadWordsNum value is: '.$whi.$jfreu->badwordsnum.$blu.'.';
			$aseco->client->query('ChatSendServerMessageToLogin', $message, $login);
		}
	}
	...
	elseif ($command['params'][0] == 'badwordstime')
	{
		if ($command['params'][1] != '' && is_numeric($command['params'][1]))
		{
			...
			{
				$jfreu->badwordstime = $command['params'][1];
				// log console message
				$aseco->console('{1} [{2}] set badwordstime: {3}', $logtitle, $login, $jfreu->badwordstime);
				// show chat message
				$message = formatText('{#server}>> {#message}{1}$z$s {#highlite}{2}$z$s{#message} set BadWordsTime to: {#highlite}{3}{#message} mins.',
				                      $chattitle, $admin->nickname, $jfreu->badwordstime);
				$aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));
				write_config_xml($aseco);
			}
		}
		...
		{
			$message = $yel.'> '.$blu.'BadWordsTime value is: '.$whi.$jfreu->badwordstime.$blu.' mins.';
			$aseco->client->query('ChatSendServerMessageToLogin', $message, $login);
		}
	}
	...
	elseif ($command['params'][0] == 'unban')
	{
		...
		if (isset($jfreu->playerlist[$target->login])) {
			if ($jfreu->playerlist[$target->login]->banned > 0) {
				$jfreu->playerlist[$target->login]->banned = 0;
	...
	elseif ($command['params'][0] == 'nopfkick')
	{
		if ($command['params'][1] != '')
		{
			...
			elseif (isset($jfreu->pf_list[$command['params'][1]]))
			{
				$jfreu->pf = $jfreu->pf_list[$command['params'][1]];
			...
			// log console message
			$aseco->console('{1} [{2}] set NoPfKick: {3}', $logtitle, $login, $jfreu->pf);
		}
		else
		{
			$message = $yel.'> '.$blu.'NoPfKick is: '.$whi.
			           ($jfreu->pf == 0 ? 'OFF' : 'ON').$blu.'.';
			$aseco->client->query('ChatSendServerMessageToLogin', $message, $login);
		}
	}
	...
	elseif ($command['params'][0] == 'infomessages')
	{
		if ($command['params'][1] != '')
		{
			...
			{
				$jfreu->infomessages = $command['params'][1];
				// log console message
...
function event_jfreu($aseco, $answer) {
	...
	elseif ($answer[2] >= -5600 && $answer[2] <= -5401) {
		...
		$bansleft = false;
		$time = time();
		foreach ($jfreu->playerlist as $entry) {
			if ($entry->banned > $time) {
				$bansleft = true;
				break;
			}
	...
	elseif ($answer[2] >= -6000 && $answer[2] <= -5801) {
		...
		$teamsleft = false;
		foreach ($jfreu->vip_team_list as $tm) {
			if ($tm != '') {
				$teamsleft = true;
				break;
			}
