...
Aseco::registerEvent('onPlayerDisconnect', 'mistral_onPlayerDisconnect');
...
global $kickPlayAfter, $kickSpecAfter, $kickSpecToo, $specPlayFirst, $resetOnChat,
       $resetOnCheckpoint, $resetOnFinish, $idlekickStart, $idlekick_log, $idlekick_debug, $mistral_idlecount;
...
function mistral_onPlayerDisconnect($aseco, $player) {
	global $mistral_idlecount;

	unset($mistral_idlecount[$player->login]);
}

// called @ onChat
function kickIdleChat($aseco, $chat) {
	global $resetOnChat, $idlekick_debug, $mistral_idlecount;
	...
	$mistral_idlecount[$player->login] = 0;
	if ($idlekick_debug)
...
function kickIdleCheckpoint($aseco, $checkpt) {
	global $resetOnCheckpoint, $idlekick_debug, $mistral_idlecount;
...
function kickIdleFinish($aseco, $finish_item) {
	global $resetOnFinish, $idlekick_debug, $mistral_idlecount;
...
function kickIdleNewChallenge($aseco, $challenge) {
	global $kickSpecToo, $idlekickStart, $idlekick_debug, $idlekick_log, $mistral_idlecount;
	...
	foreach ($aseco->server->players->player_list as $player) {
		...
		if ($kickSpecToo || !$spec)
			$mistral_idlecount[$player->login]++;
		if ($idlekick_log)
			$aseco->console('Idlekick: {1} set to {2}', $player->login, $mistral_idlecount[$player->login]);
	}
}  // kickIdleNewChallenge

// called @ onPlayerConnect
function kickIdleInit($aseco, $player) {
	global $idlekick_debug, $mistral_idlecount;
...
function kickIdlePlayers($aseco, $data) {
	global $kickPlayAfter, $kickSpecAfter, $specPlayFirst, $idlekick_debug, $mistral_idlecount;

	foreach ($aseco->server->players->player_list as $player) {
		$spec = $aseco->isSpectator($player);
		// check for spectator or player challenge counts
		if ($mistral_idlecount[$player->login] == ($spec ? $kickSpecAfter
		                                            : $kickPlayAfter)) {
			$dokick = false;
			if ($spec) {
				$dokick = true;
		...
		elseif ($idlekick_debug)
			$aseco->console('Idlekick: {1} current value is {2}', $player->login, $mistral_idlecount[$player->login]);
	}
}  // kickIdlePlayers
?>
