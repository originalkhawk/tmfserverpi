...
function formatTimeH($MwTime, $hsec = true) {
	...
	if ($MwTime == -1) {
		...
		$hseconds = substr($MwTime, strlen($MwTime)-3, 2);
		$MwTime = intval(substr($MwTime, 0, strlen($MwTime)-3));
...
function quotedString($input) {
	...
	global $aseco;
	return "'" . $aseco->db->real_escape_string($input) . "'";
}  // quotedString
...
function handleSpecialChars($string, $illuminate = false)
{
    // Replace two dollar signs with sth else temporarily, so the regex works correctly
    $string = str_replace('$$', "\0", $string);

    // Remove links, e.g. "$(L|H|P)[...]...$(L|H|P)"
    $string = preg_replace('/\$[LHP](?>\[[^\]]*\])?(.*?)\$[LHP]/i', '$1', $string);
    $string = preg_replace('/\$[LHP](?>\[[^\]]*\])?/i', '', $string);
    ...
    // Remove $O (bold)
    // Remove $N (narrow)
    $string = preg_replace('/\$[SHWILON]/i', '', $string);


    if ($illuminate) {
        // Replace too dark colors with lighter ones
        $string = str_replace(['$000', '$111', '$222', '$333', '$444', '$555'], '$AAA', $string);
    }
    ...
    return validateUTF8String($string);
}
?>
