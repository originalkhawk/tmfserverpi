	...
	if ($feature_jukebox && $feature_tmxadd) {
		...
		if (is_numeric($command['params'][0]) && $command['params'][0] >= 0) {
			...
			if ($file === false || $file == -1) {
				...
				$filename = trim(mb_convert_encoding(stripColors($newtrk['name']), 'ISO-8859-1', 'UTF-8'));
...
function chat_y($aseco, $command) {
	...
	if (is_array($plrvotes) && in_array($login, $plrvotes)) {
...
function event_jukebox($aseco, $answer) {
	...
	elseif ($answer[2] >= -200000 && $answer[2] <= -100001) {
		// get player
		$player = $aseco->server->players->getPlayer($answer[1]);
		$author = $player->tracklist[abs($answer[2])-100001]['author'];
